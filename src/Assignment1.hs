...
longerList::[a] -> [a] -> Bool
longerList x y = length x > length y 
...
insert f x [] = [x]
insert f x (y:ys) | f x > f y = y: insert f x ys
                  | otherwise = x:y:ys
...
inssort f [] = []
inssort f (x:ys) = insert f x (inssort f ys)
...
bit2Int [O] = 0
bit2Int [I] = 1
bit2Int (y:ys) = (bit2Int [y] * 2^length ys) + bit2Int ys
...
remNth _ [] = []
remNth 0 xs = xs
remNth n xs = take (n-1) xs ++ remNth n (drop n xs)
...
revHelper x y | x == 0 = y
              | otherwise = revHelper (x `div` 10) ((y*10) + (x `rem` 10))

revDig::Int-> Int
revDig 0 = 0
revDig x | x < 0 = revDig ((-1) * x) * (-1)
         | otherwise = revHelper x 0
